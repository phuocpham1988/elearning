    ...
    public function dequy_showLesson($data)
    {
        ...
        foreach ($records as $key => $r) {
            if ($r->parent_id == $parent_id) {
                # pre check
                $class_color   = ($content_view[0]->stt >= $r->stt || $lms_type == 1) ? '#28a745' : '#9e9e9e';
                $class_i_color = ($content_view[0]->stt >= $r->stt || $lms_type == 1) ? '#2a93e2' : '#000';

                $is_active = ($r->id == $stt) ? 'lesson_active' : null;

                if (in_array($r->type, $array_video)) {

                    ($content_view[0]->stt >= $r->stt || $lms_type == 1) ? $i_tag = '<img src="/public/assets/images/icon-seriess/play.png" style="width: 20px;margin-right: 5px;" alt="play">' : $i_tag = '<img src="/public/assets/images/icon-seriess/sand-clock.png" style="width: 20px;margin-right: 5px;" alt="sand-clock">';

                    $video_url = ($content_view[0]->stt >= $r->stt || $lms_type == 1)
                ...
                if ($r->type == 5) {
                    ($content_view[0]->stt >= $r->stt || $lms_type == 1) ? $i_tag = '<img src="/public/assets/images/icon-seriess/compliant.png" style="width: 20px;margin-right: 5px;" alt="checklist">' : $i_tag = '<img src="/public/assets/images/icon-seriess/sand-clock.png" style="width: 20px;margin-right: 5px;" alt="sand-clock">';
                    $video_url                                                    = ($content_view[0]->stt >= $r->stt || $lms_type == 1)
                    ? PREFIX . 'learning-management/lesson/audit/' . $combo_slug . '/' . $slug . '/' . $r->id : '#';
                }

                if ($r->type == 10) {
                    ($content_view[0]->stt >= $r->stt || $lms_type == 1) ? $i_tag = '<img src="/public/assets/images/icon-seriess/flash-cards.png" style="width: 20px;margin-right: 5px;" alt="checklist">' : $i_tag = '<img src="/public/assets/images/icon-seriess/sand-clock.png" style="width: 20px;margin-right: 5px;" alt="sand-clock">';
                    $video_url                                                    = ($content_view[0]->stt >= $r->stt || $lms_type == 1)
                    ? PREFIX . 'learning-management/lesson/flashcard/' . $combo_slug . '/' . $slug . '/' . $r->id : '#';
                }

                $i_viewed = ($content_view[0]->stt > $r->stt || $lms_type == 1)
          ...
          ';
            ...
            ';
                ...
                $child = $this->dequy_showLesson(['content_q' => $records, 'content_view' => $content_view,
                    'stt'                                         => $stt, 'slug'             => $slug, 'parent_id' => $r->id, 'combo_slug' => $combo_slug, 'lms_type' => $lms_type]);
                $result = $result + $child;
            }
        }
        return $result;
    ...
    public function flashCard(Request $request, $combo_slug = '', $slug = '', $stt = '')
    {
        ...
        try {
            ...
            $records = DB::table('lms_flashcard')
                ...
                ->join('lmscontents', 'lmscontents.flashcard_id', 'lms_flashcard.id')
                ->where('lmscontents.id', $stt)
