    ...
    */
    ...
    public function socialLoginCancelled(Request $request)
    {
     ...
     public function resetUsersPassword(Request $request)
     {
        ...
        try{
        sendEmail('forgotpassword', array('user_name'=>$user->email, 'username'=>$user->email, 'to_email' => $user->email, 'password'=>$password, 'confirmation_link' => ''));
          }
         catch(Exception $ex)
        {
        }

           flash('Thành công', 'Mật khẩu mới đã được gửi về email', 'success');

         }

         else{

            flash('Có lỗi','Email này không tồn tại trong hệ thống','error');
